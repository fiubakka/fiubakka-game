	...
	match play_type:
		PBTrucoPlayTypeEnum.CARD:
			...
			truco_play_card.emit(
				play_id, suit, rank, player_cards, game_over, match_over, first_points, first_name, second_points, second_name
			)
		...
		PBTrucoPlayTypeEnum.UPDATE:
			...
			truco_play_update.emit(
				play_id, player_cards, game_over, match_over, first_points, first_name, second_points, second_name
			)
