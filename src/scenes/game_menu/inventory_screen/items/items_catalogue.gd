...
var items_catalogue := {
	"miscellaneous": {}, "hats": {}, "outfits": {}, "hairs": {}, "facial hair": {}, "glasses": {}
}
...
func create_resources(
	type: String, original_res: InventoryItemData, texture_dic: Dictionary
) -> void:
...
func create_new_resource(
	index: int, type: String, original_res: InventoryItemData, texture_dic: Dictionary
) -> void:
	if texture_dic[index]:
		var new_res := original_res.duplicate(true)
		if type == "hats":
			new_res.name = tr("COSMETIC_NAME_HAT_" + str(index))
		print("type:", type)
